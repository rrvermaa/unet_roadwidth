...
def test_load_dataset(path):
    ...
    test_y = sorted(glob(os.path.join(path, "images","*")))[:160]
    # train_y = sorted(glob(os.path.join(path,  "SegmentationClassPNG", "*")))[:160]
...
def save_results(image, pred, save_image_path):
    ...
    cat_images = np.concatenate([image, line, line, pred], axis=1)
...
if __name__ == "__main__":
    ...
    """ Load the dataset """
    test_x, test_y =  test_load_dataset(dataset_path)
